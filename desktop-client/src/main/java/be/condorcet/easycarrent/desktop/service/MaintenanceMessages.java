...
/**
 * JavaFX-free formatting of safe, user-facing messages for the Maintenance screen.
 *
 * <p>Turns backend failures into short, readable text with one issue per line,
 * deterministic ordering, and no duplicates. Backend conflict messages for
 * maintenance (overlapping maintenance or rental, inactive vehicle, wrong-status
 * transition, vehicle-status mismatch, and the delete rule) are safe and useful and
 * carry no personal data, so they are shown as-is; connection and unexpected
 * failures map to fixed safe text. It never exposes raw JSON, exception-class names,
 * stack traces, SQL, {@code Authorization} values, passwords, or customer data.
 * Keeping this logic out of the controller makes it independently testable.</p>
 */
public final class MaintenanceMessages {
	...
	public static final String RECORD_NOT_FOUND =
			"The maintenance record no longer exists. Please refresh.";
	public static final String VEHICLE_NOT_FOUND =
			"The selected vehicle no longer exists. Please refresh.";
	...
	private static final List<String> FIELD_ORDER =
			List.of("vehicleId", "description", "startDate", "endDate", "cost");
	 ...
	 * line, while any other failure (invalid period, overlap, inactive vehicle,
	 * missing vehicle, connection, unexpected) is a single safe line. All belong below
	 * the form, never above the table.
	 */
	...
	/** Safe message for a failed create (a missing vehicle is 404). */
	...
	public static String forDeleteFailure(Throwable throwable) {
		return forRecordOperationFailure(throwable);
	}

	/** Safe message for a failed lifecycle transition (start, complete). */
	public static String forTransitionFailure(Throwable throwable) {
		return forRecordOperationFailure(throwable);
	}

	private static String forRecordOperationFailure(Throwable throwable) {
		Throwable cause = unwrap(throwable);
		if (cause instanceof ApiConnectionException) {
			return CONNECTION_UNAVAILABLE;
		}
	...
	private static String generalRequest(ApiRequestException request) {
		...
		if (status == HttpURLConnection.HTTP_BAD_REQUEST) {
			return request.apiError().map(MaintenanceMessages::backendValidation).orElse(REQUEST_INVALID);
		}
