...
/**
 * JavaFX-free formatting of safe, user-facing messages for the Payments screen.
 *
 * <p>Turns backend failures into short, readable text with one issue per line,
 * deterministic ordering, and no duplicates. Backend conflict messages for payments
 * (duplicate payment, non-payable rental, wrong-status transition, and the delete
 * rule) are safe and useful and carry no personal or financial-sensitive data, so
 * they are shown as-is; connection and unexpected failures map to fixed safe text.
 * It never exposes raw JSON, exception-class names, stack traces, SQL,
 * {@code Authorization} values, passwords, or customer data. Keeping this logic out
 * of the controller makes it independently testable.</p>
 */
public final class PaymentMessages {
	...
	public static final String PAYMENT_NOT_FOUND = "The payment no longer exists. Please refresh.";
	public static final String RENTAL_NOT_FOUND =
			"The selected rental no longer exists. Please refresh.";
	...
	private static final List<String> FIELD_ORDER = List.of("rentalId", "paymentMethod");
	...
	/**
	 * Every create failure as a list of lines for the form message area: backend
	 * field errors (HTTP 400 with {@code validationErrors}) are aggregated one per
	 * line, while any other failure (duplicate payment, non-payable rental, missing
	 * rental, connection, unexpected) is a single safe line. All belong below the
	 * form, never above the table.
	 */
	...
	/** Safe message for a failed create (a missing rental is 404). */
	...
	public static String forDeleteFailure(Throwable throwable) {
		return forPaymentOperationFailure(throwable);
	}

	/** Safe message for a failed lifecycle transition (pay, fail, retry, refund). */
	public static String forTransitionFailure(Throwable throwable) {
		return forPaymentOperationFailure(throwable);
	}

	private static String forPaymentOperationFailure(Throwable throwable) {
		Throwable cause = unwrap(throwable);
		if (cause instanceof ApiConnectionException) {
			return CONNECTION_UNAVAILABLE;
		}
	...
	private static String generalRequest(ApiRequestException request) {
		...
		if (status == HttpURLConnection.HTTP_BAD_REQUEST) {
			return request.apiError().map(PaymentMessages::backendValidation).orElse(REQUEST_INVALID);
		}
