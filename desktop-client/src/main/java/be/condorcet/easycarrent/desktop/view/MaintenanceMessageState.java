...
/**
 * JavaFX-free placement model for the two Maintenance message areas.
 *
 * <p>Keeps the form (Add) errors and the general status/operation message in one
 * place and enforces that the same failure is never shown in both areas: form
 ...
 * without the graphical toolkit. It reuses the lesson from the Customers, Rentals,
 * and Payments screens so a stale form error can never linger above the table.</p>
 */
public final class MaintenanceMessageState {
	...
	/** Clears both areas (used on Add open, Cancel, and before each Save pass). */
	...
	/**
	 * Shows a general error/operation message above the table (for list, delete, and
	 * transition feedback) and clears any form errors.
	 */
